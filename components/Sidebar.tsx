...
interface SidebarProps {
  currentView: 'landing' | 'dashboard' | 'chat' | 'finetuning' | 'resources' | 'wp_dashboard' | 'teamyar_erp' | 'checklists' | 'vegetation' | 'hydromap';
  onViewChange: (view: 'landing' | 'dashboard' | 'chat' | 'finetuning' | 'resources' | 'wp_dashboard' | 'teamyar_erp' | 'checklists' | 'vegetation' | 'hydromap') => void;
  isOpen?: boolean;
  onClose?: () => void;
    ...
    <>
      ...
      <nav className="px-4 py-4">
          <ul className="space-y-1">
              ...
              <li>
                  <button onClick={() => handleNavClick('hydromap')} className={`w-full ${isRTL ? 'text-right' : 'text-left'} px-4 py-3 rounded-xl text-sm font-medium transition-all flex items-center gap-3 ${currentView === 'hydromap' ? 'bg-slate-700 text-sky-300' : 'text-gray-400 hover:text-white'}`}>
                      <span className="text-lg">🛰️</span> {t('nav_hydromap')}
                  </button>
              </li>
              <li>
